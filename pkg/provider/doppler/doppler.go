...
const (
	...
	errDopplerStore                                    = "missing or invalid Doppler SecretStore received"
	errDopplerTokenSecretName                          = "invalid Doppler Secret Store: auth.secretRef.dopplerToken.name not specified"
	errInvalidClusterStoreMissingDopplerTokenNamespace = "invalid Doppler Cluster Store: missing DopplerToken namespace"
	errFetchDopplerTokenSecret                         = "couldn't find DopplerToken secret: %w"
	errMissingDopplerToken                             = "the DopplerToken ref key '%s' could not be found in '%s'"
...
type SecretsClientInterface interface {
	...
	DownloadSecrets(request client.SecretsDownloadRequest) (*client.SecretsDownloadResponse, error)
}
...
func (d *Doppler) NewClient(ctx context.Context, store esv1beta1.GenericStore, kube kclient.Client, namespace string) (esv1beta1.SecretsClient, error) {
	...
	d.includeDynamicSecrets = kubeClient.store.IncludeDynamicSecrets
	d.dynamicSecretsTTL = kubeClient.store.DynamicSecretsTTL
...
func (d *Doppler) ValidateStore(store esv1beta1.GenericStore) error {
	...
	// default Key to dopplerToken if not supplied
	if dopplerTokenSecretRef.Key == "" {
		dopplerTokenSecretRef.Key = "dopplerToken"
	}
...
func (d *Doppler) GetSecrets(_ context.Context) (map[string][]byte, error) {
	if d.format != "" {
		request := client.SecretsDownloadRequest{
			Project:               d.project,
			Config:                d.config,
			Format:                d.format,
			NameTransformer:       d.nameTransformer,
			IncludeDynamicSecrets: d.includeDynamicSecrets,
			DynamicSecretsTTL:     d.dynamicSecretsTTL,
		}

		response, err := d.client.DownloadSecrets(request)
		if err != nil {
			return nil, fmt.Errorf(errGetSecrets, err)
		}

		return map[string][]byte{
			secretsDownloadFileKey: []byte(response.File),
		}, nil
	}
