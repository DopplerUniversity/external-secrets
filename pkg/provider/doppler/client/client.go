...
type SecretsDownloadResponse struct {
	File     string
	Modified bool
	ETag     string
}
...
func NewDopplerClient(dopplerToken string) (*DopplerClient, error) {
	client := &DopplerClient{
		VerifyTLS: true,
		UserAgent: "doppler-go-secrets-client",
	}
	...
	client.DopplerToken = dopplerToken
	return client, nil
}
...
func (c *DopplerClient) Authenticate() error {
	//  Chose projects as a lightweight endpoint for testing authentication
...
func (c *DopplerClient) GetSecrets(request SecretsRequest) (*SecretsResponse, error) {
	...
	return &SecretsResponse{Modified: modified, Secrets: secrets, ETag: eTag}, nil
}

func (c *DopplerClient) DownloadSecrets(request SecretsDownloadRequest) (*SecretsDownloadResponse, error) {
	if request.Format == "" {
		return nil, &APIError{Message: "no download format provided"}
	}
	...
	return &SecretsDownloadResponse{Modified: modified, File: file, ETag: eTag}, nil
}
...
func (r *SecretsRequest) buildQueryParams() queryParams {
	...
	if r.IncludeDynamicSecrets {
		params["include_dynamic_secrets"] = strconv.FormatBool(r.IncludeDynamicSecrets)
		params["dynamic_secrets_ttl_sec"] = strconv.FormatInt(r.DynamicSecretsTTL, 10)
	}

	return params
}

func (r *SecretsDownloadRequest) buildQueryParams() queryParams {
	params := queryParams{}
...
func sortSecrets(unsorted Secrets) Secrets {
	names := make([]string, 0, len(unsorted))
	sorted := make(Secrets, len(unsorted))

	for name := range unsorted {
